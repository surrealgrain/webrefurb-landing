...
- GLM-owned visual designs are preserved; Codex normalized contract metadata, safety notes, and renderer routing.
- Codex routing maps `establishment_profile` IDs below to the template files in `pipeline/outreach.py` and `pipeline/state_audit.py`.
- No live records should be promoted or made send-ready by this template work alone.
...
## Legacy Templates Now On Shared Contract
...
- `data-slot` attributes on editable elements: `seal`, `seal-text`, `panel-title` where applicable, `section-title`, `section-items`, `item`, `button-grid`, or `qr-code`.
- A4 print styles and mobile breakpoint included where the output is print or web visible.
- Customer-facing footer note present: confirms sample is illustrative and states owner-confirmed content/photos are used in production.
...
Run 5 previews are generated through the structured HTML renderer and saved under `state/template-previews/run5/`; public proof images are copied to `docs/assets/previews/`.
